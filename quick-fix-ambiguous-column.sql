-- ============================================================================
-- QUICK FIX: Resolve ambiguous column reference error
-- ============================================================================
-- This fixes the "column reference 'is_available' is ambiguous" error
-- and "column reference 'slot_time' is ambiguous" error
-- that occurs when trying to book appointments
...
DROP FUNCTION IF EXISTS validate_appointment_booking() CASCADE;
DROP FUNCTION IF EXISTS mark_slot_unavailable_after_booking() CASCADE;
DROP FUNCTION IF EXISTS mark_slot_available_after_cancellation() CASCADE;

-- ============================================================================
-- STEP 1: Create a function to validate appointment bookings (FIXED)
-- ============================================================================
...
BEGIN
    ...
    -- IMPORTANT: Only admin-created availability slots are valid for booking
    SELECT EXISTS(
    ...
    -- This means admin has not scheduled this doctor for this time
    IF NOT slot_available THEN
        RAISE EXCEPTION 'Cannot book appointment: Doctor % has not been scheduled for % at %. Please contact admin to schedule availability.', 
...
BEGIN
    ...
    RAISE NOTICE 'All ambiguous column reference errors have been resolved.';
    RAISE NOTICE 'Appointment validation is now working properly.';
END $$;
